import{t as e}from"./style-D74fJMD3.js";e((()=>{document.addEventListener(`DOMContentLoaded`,async()=>{let e=new URLSearchParams(window.location.search).get(`id`),t=document.getElementById(`details-container`);if(!e){t.innerHTML=`<div style="padding: 10rem 0; text-align: center;"><h2>Vehicle not found</h2><a href="./index.html" class="btn outline" style="margin-top: 2rem;">Return to Inventory</a></div>`;return}try{let n=await fetch(`/api/inventory/${e}`);if(!n.ok)throw Error(`Failed to fetch`);let r=await n.json(),i=r.images?r.images:r.image?[r.image]:[],a=i.length>0?i[0]:``,o=e=>e&&e.toLowerCase().match(/\.(mp4|webm|ogg)$/i),s=(e,t=!1,n=0)=>{let r=e;return(r.startsWith(`./`)||r.startsWith(`/`))&&(r=``+r.replace(/^\.\//,`/`)),o(r)?t?`<video src="${r}" class="${n===0?`active`:``}" onclick="changeMainImage('${r}', this, true)" muted playsinline></video>`:`<video id="main-display-img" src="${r}" autoplay loop muted playsinline></video>`:t?`<img src="${r}" class="${n===0?`active`:``}" onclick="changeMainImage('${r}', this, false)">`:`<img id="main-display-img" src="${r}" alt="Vehicle Media">`},c=``;i.length>1&&(c=`<div class="gallery-thumbs">`,i.forEach((e,t)=>{c+=s(e,!0,t)}),c+=`</div>`),t.innerHTML=`
